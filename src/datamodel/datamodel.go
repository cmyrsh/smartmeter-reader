...
type P1Telegram struct {
	...
	Equipment_id                          string
	Electricity_incoming_tariff1_kwh      float64
	Electricity_outgoing_tariff1_kwh      float64
	Electricity_incoming_tariff2_kwh      float64
	Electricity_outgoing_tariff2_kwh      float64
	Tarrif_id                             string
...
func (target *P1Telegram) PopulateFromLine(data string, debug bool) error {
	...
	if strings.HasPrefix(str_e, const_p_eto_trf1) {
		...
		target.Electricity_incoming_tariff1_kwh, _ = strconv.ParseFloat(kWhremove.ReplaceAllString(p_eto_trf1.Split(str_e, -1)[1], ""), 0)
		return nil
	}
	if strings.HasPrefix(str_e, const_p_efrom_trf1) {
		target.Electricity_outgoing_tariff1_kwh, _ = strconv.ParseFloat(kWhremove.ReplaceAllString(p_efrom_trf1.Split(str_e, -1)[1], ""), 0)
		return nil
	}
	if strings.HasPrefix(str_e, const_p_eto_trf2) {
		//log.Println("const_p_eto_trf2 : ", str_e)
		target.Electricity_incoming_tariff2_kwh, _ = strconv.ParseFloat(kWhremove.ReplaceAllString(p_eto_trf2.Split(str_e, -1)[1], ""), 0)
		return nil
	}
	if strings.HasPrefix(str_e, const_p_efrom_trf2) {
		target.Electricity_outgoing_tariff2_kwh, _ = strconv.ParseFloat(kWhremove.ReplaceAllString(p_efrom_trf2.Split(str_e, -1)[1], ""), 0)
		return nil
	}
